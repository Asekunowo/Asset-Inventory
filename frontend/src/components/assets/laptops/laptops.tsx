...
import {Table, TableBody, TableColumnHeader, TableHeader, TableRow} from "@chakra-ui/react";

const Laptops = ({laptops}: any) => {
	return (
		<Table.ScrollArea borderWidth="1px" maxW="9xl" colorPalette={"gray"}>
			<Table.Root borderRadius={"md"} stickyHeader variant={"outline"} showColumnBorder>
				<TableHeader>
					<TableRow>
					...
					{laptops.map((asset: Asset, index: number) => {
						return (
							<TableRow key={index}>
								...
								<TableColumnHeader textAlign={"end"}>{asset.branch}</TableColumnHeader>
								<TableColumnHeader>{asset.createdAt}</TableColumnHeader>
								<TableColumnHeader bg={"gray.300"} fontWeight={"bold"} color={"gray.500"}>
									{asset.createdBy!.firstname || undefined + " " + asset.createdBy!.lastname || undefined}
								</TableColumnHeader>
							</TableRow>
						);
					})}
